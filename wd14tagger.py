...
async def download_model(model, client_id, node):
    if os.path.exists(f'/stable-diffusion-cache/models/wd14_tagger/{model}.onnx'):
        os.system(f'cp /stable-diffusion-cache/models/wd14_tagger/{model}.onnx {models_dir}/{model}.onnx')
        os.system(f'cp /stable-diffusion-cache/models/wd14_tagger/{model}.csv {models_dir}/{model}.csv')
